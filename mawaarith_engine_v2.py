...
class MawaarithEngine:
    ...
    def compute(self, estate: Estate) -> Result:
        ...
        # Son alone (no daughter) → pure Asaba, takes full residue
        elif has_son and not has_daughter:
            furud_others = sum(v for k, v in shares.items() if k != "son")
            shares["son"] = max(Fraction(1) - furud_others, Fraction(0))
            residue = Fraction(0)
        ...
        if total_shares < Fraction(1) and not awl_applied:
            ...
            elif "husband" in shares or "wife" in shares:
                # No non-spouse Furud/Asaba heir exists — spouse is the sole
                # surviving heir. In the absence of any other claimant (and
                # absent a functioning Bayt al-Mal/state treasury to escheat
                # to), the full residue reverts to the spouse by Radd.
                surplus = Fraction(1) - total_shares
                spouse_role = "husband" if "husband" in shares else "wife"
                shares[spouse_role] += surplus
                radd_applied = True
                notes.append(f"Radd applied: spouse is sole heir — surplus {surplus} reverts to spouse (no other claimant).")
